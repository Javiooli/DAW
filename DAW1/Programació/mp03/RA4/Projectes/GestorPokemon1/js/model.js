...
class Pokemon {
    ...
    constructor (id, name, image, points, types, specialPower) {
        ...
        this._points = points;
        // Si el Pokémon té més d'un tipus, es concatenen en una sola cadena, sinó, es guarda un sol tipus
        this._types = types.length > 1
                        ? `${types[0]}/${types[1]}`
                        : types[0];
...
export class PokemonTeam {
    ...
    addPokemon(pokemon) {
        if (pokemon && pokemon.points <= this._credits && this._selectedTeam.find(p => p.name === pokemon.name) === undefined) { // Comprovem que entra pokémon per paràmetre, que tenim punts suficients i que el pokémon no es trobi ja a l'equip.
            ...
            try {
                ...
                pokemons.forEach(pokemon => {
                    ...
                    const instancia = new PokemonClass(
                        ...
                        "image_placehonder.png", // Assigna la imatge
