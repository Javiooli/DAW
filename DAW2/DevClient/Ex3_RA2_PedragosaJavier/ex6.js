/*
    ...
    Requisits:
    Sincronització correcta entre array i Map
*/
...
function test(_args) {    
    ...
    for (const product of productNames) {
        products.push(new Item(product, 20 + Math.round(Math.random() * 30)));
    }
    const inventari = new Inventari(products);

    // Mostrar l'estat inicial del Map
    console.log("\n--- 1. ESTAT INICIAL ---");
    console.log("\nArray d'items:");
    inventari.llistarProductes();
    console.log("\nMap d'estoc:");
    alert(inventari.mostrarMap());
    console.log("Sincronitzats? " + inventari.estasSincronitzats());

    // Modificar l'estoc directament al Map
    console.log("\n--- 2. MODIFICAR ESTOC AL MAP ---");
    inventari.modificarEstocAlMap("Poma", 100);
    inventari.modificarEstocAlMap("Tablet", 5);
    console.log("\nMap després de modificacions:");
    alert(inventari.mostrarMap());
    console.log("\nArray (encara no actualitzat):");
    inventari.llistarProductes();
    console.log("Sincronitzats? " + inventari.estasSincronitzats());

    // Sincronitzar: actualitzar array des del Map
    console.log("\n--- 3. ACTUALITZAR ARRAY DES DEL MAP ---");
    inventari.actualitzarArrayDesDeMap();
    console.log("\nArray després de la sincronització:");
    inventari.llistarProductes();
    console.log("Sincronitzats? " + inventari.estasSincronitzats());

    // Fer compres (modifica l'array)
    console.log("\n--- 4. FER COMPRES (MODIFICA L'ARRAY) ---");
    try {
        inventari.comprar("Poma", 15);
        inventari.comprar("Sabata", 10);
    } catch (e) {
        console.log("Error en compra: " + e.message);
    }
    console.log("\nArray després de les compres:");
    inventari.llistarProductes();
    console.log("\nMap (encara no actualitzat):");
    alert(inventari.mostrarMap());
    console.log("Sincronitzats? " + inventari.estasSincronitzats());

    // Sincronitzar: actualitzar Map des de l'array
    console.log("\n--- 5. ACTUALITZAR MAP DES DE L'ARRAY ---");
    inventari.actualitzarMapDesDeArray();
    console.log("\nMap després de la sincronització:");
    alert(inventari.mostrarMap());
    console.log("Sincronitzats? " + inventari.estasSincronitzats());
    ...
    console.log("\nMap:");
    alert(inventari.mostrarMap());
    console.log("\n✓ Sincronització bidireccional completada correctament!");
}
